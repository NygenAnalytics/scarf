...
Counts are streamed in blocks; RNA assays require a strip-sharded feature-major `countsT`
orientation for gene-wise stages such as highly variable feature selection and marker search; and
`mem_budget` shapes block geometry, write concurrency, and automatically sized feature batches.
