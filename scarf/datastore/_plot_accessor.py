...
class DataStorePlotAccessor:
    ...
    def violinplot(
        ...
        *,
        ...
        from ..plotting import violinplot
    ...
    def boxplot(
        ...
        *,
        ...
        Without ``facet_by`` this draws a single box plot per gene (x-axis is
        the ``group_by`` categories); pass ``facet_by`` to split each gene
        into a faceted grid instead. No legend is drawn by default since the
        ``group_by`` categories are labelled on the x-axis; pass
        ``show_legend=True`` to add one."""
        ...
        return boxplot(
            ...
            panel_label_format=panel_label_format,
            cell_key=cell_key,
            from_assay=from_assay,
            normalization=normalization,
            groups=groups,
    ...
    def stacked_violin(
        ...
        *,
        ...
        Genes form a vertical stack of slim subplots with zero spacing; the
        ``group_by`` categories line up as columns, gene names sit on the left,
        and x-axis ticks appear only on the bottom row. Violins are coloured by
        mean expression by default (``color_by="mean"``); pass
        ``color_by="group"`` for shared group colours instead."""
        from ..plotting import stacked_violin
